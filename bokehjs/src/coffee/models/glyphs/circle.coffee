...
class CircleView extends Glyph.View
  ...
  draw_legend: (ctx, x0, x1, y0, y1, reference_index) ->
    indices = [reference_index]
    sx = { }
    sx[reference_index] = (x0+x1)/2
    sy = { }
    sy[reference_index] = (y0+y1)/2
    sradius = { }
    sradius[reference_index] = Math.min(Math.abs(x1-x0), Math.abs(y1-y0))*0.2
